...
class Block:
    ...
    def move(self): # 볼 움직임
        ...
        # 볼이 움직이는 y축 값을 계속 계산하려면  y는 dir 값을 라디언으로 변환 후 사인 처리
...
pygame.init()
Surface = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGTH))
FPSCLOCK = pygame.time.Clock()
pygame.display.set_caption('Paygame Blocks!!')
...
def main():
    ...
    score = 0
    BLOCK = []
    BALL = Block((200, 200 ,0), Rect(375, 650, 20, 20), 15) # 공 생성, 스피드 15으로 시작
    PADDLE = Block((200, 200 ,0), Rect(375, 700, 100, 30)) # 공을 맟출 패달을 생성
    ...
    while True:
        ...
        else: # 게임 시작 후 블록 다 그리고 볼이 움직이게 처리, 바도 움직이도록 
            Surface.blit(M_SCORE, (25, 760))
            Surface.blit(M_SPEED, ((SCREEN_WIDTH - 220), 760))
            lenBlock = len(BLOCK) # 54개으로 시작하지만 공에 충돌해서 개수가 계속 줄어듦
